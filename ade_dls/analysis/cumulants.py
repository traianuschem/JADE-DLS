...
#plot Gamma vs. q^2
def analyze_diffusion_coefficient(data_df, q_squared_col, gamma_cols, method_names=None, gamma_unit='1/s', x_range=None, show_plots=True, fit_through_origin=False):
    #validate inputs
    ...
    for i, gamma_col in enumerate(gamma_cols):
        ...
        #plot all data points (extend x-axis to 0 when forcing through origin)
        x_plot_min = 0.0 if fit_through_origin else float(X_full.min())
        ...
        if fit_through_origin:
            #forced-through-origin fit — Γ = D·q² with intercept fixed at 0.
            #slope = Σxy/Σx², all statistics derived from this fit only.
            ...
            _, jb_p_value      = stats.jarque_bera(residuals)
            _, omnibus_p_value = stats.normaltest(residuals)
            jb_p_value      = float(jb_p_value)
            omnibus_p_value = float(omnibus_p_value)

            X_line = np.linspace(0.0, float(X_full.max()), 100)
            plt.plot(X_line, slope * X_line, 'b-', label=f'OLS through origin{fit_range_text}')
            ...
            X_line = np.linspace(x_plot_min, float(X_full.max()), 100)
            X_line_with_constant = sm.add_constant(X_line)
            plt.plot(X_line, model.predict(X_line_with_constant), 'r-', label=f'Regression Line{fit_range_text}')
        ...
        #add coefficients and standard errors (slope keyed by q² column for downstream D lookup)
        results_dict[f'{q_squared_col}_coef'] = slope
        results_dict[f'{q_squared_col}_se']   = slope_se
        results_dict['const_coef'] = intercept
        results_dict['const_se']   = intercept_se
