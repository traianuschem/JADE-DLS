...
- **Cumulant C 4th-order initial guess** (`ade_dls/analysis/cumulants_C.py`): the starting value for the 4th cumulant parameter `e` in `estimate_parameters_from_data()` was `0.5 * c_estimate**2`; restored to the Gaussian-distribution prior `3 * c_estimate**2` (μ₄ ≈ 3·μ₂²) used by the original JADE-DLS notebook. Only affects `fit_function4` (4th-order fits); `fit_function1–3` were already numerically correct.
- **Regularized peak statistics and normalization** (`ade_dls/analysis/regularized_optimized.py`): the actively-used `regularized_nnls_optimized()` returned only `tau_i`/`intensity_i` per peak — no area, shape statistics, or SLS-ready intensity weighting. Ported the full peak-statistics block from the original JADE `regularized.py` (`area_i`, `normalized_area_percent_i`, `normalized_sum_percent_i`, `fwhm_i`, `skewness_i`, `kurtosis_i`, `std_dev_i`, `centroid_i`), with peak area integrated in **log-τ space** (`np.trapezoid(y, np.log(τ))`), matching the log-spaced decay-time grid. `nnls_optimized()`'s peak-area integration was switched from linear to the same log-τ space for consistency between NNLS and Regularized.
- **Missing `fit_through_origin` option for Γ-vs-q² diffusion fits**: the original JADE-DLS `analyze_diffusion_coefficient()` supported forcing the linear regression through the origin (D = Σxy/Σx², intercept = 0); this option was absent in ADE-DLS. Re-added to `ade_dls/analysis/cumulants.py::analyze_diffusion_coefficient()` and `ade_dls/analysis/peak_clustering.py::analyze_diffusion_coefficient_robust()`, wired through all Cumulant methods A–D (`cumulant_analyzer.py`, new shared helper `_fit_gamma_vs_q2()`) and Regularized (`laplace_analyzer.calculate_regularized_diffusion_coefficients()`), with a "Fit through origin (intercept = 0)" checkbox in the Cumulant A–D dialogs and the Regularized/Method-D post-fit refinement dialogs.
- **Missing optional β-fit for Regularized NNLS**: `fit_beta` existed in the unused `ade_dls/analysis/regularized.py` module but not in the actively-used `regularized_optimized.py::regularized_nnls_optimized()`. Ported the optional coherence-factor fit (model `g²(τ)-1 = β·(∑Aᵢ·exp(-τ/τᵢ))²`, bounds 0–2) into the active path; added a "Fit beta (coherence factor β)" checkbox to `RegularizedDialog`, and β is now shown in the Regularized results data tab.
- **`min_abundance` not configurable in Regularized post-refinement**: `cluster_all_gammas()` already supported `min_abundance`, but `laplace_analyzer.calculate_regularized_diffusion_coefficients()` never forwarded it (hardcoded to the 0.3 default). Added the parameter end-to-end, with a new spinbox in `LaplacePostFitRefinementDialog`'s clustering tab.
- **Crash in Regularized post-refinement** (`ValueError: Column 'gamma_pop1' not found in DataFrame`): a second call to `calculate_regularized_diffusion_coefficients()` (post-refinement after an initial clustered run) auto-detected tau columns via a naive `startswith('tau_')` check, which also matched population-indexed `tau_pop*` columns written by the previous `cluster_all_gammas()` call. These were translated to a bogus `gamma_pop{n}` name that had just been dropped as stale, causing `cluster_all_gammas()`'s input validation to fail. Fixed by excluding `tau_pop*` from auto-detection (mirroring the already-correct NNLS code path) and broadening the stale-column cleanup to all population-indexed columns (`tau_pop*`, `intensity_pop*`, `area_pop*`, etc.), not just `gamma_pop*`.
- **`scipy.sparse.diags()` FutureWarning**: the Tikhonov regularization matrix was built from an int64 diagonal `[1, -2, 1]`, implicitly cast to float64. Added `dtype=float` at all three occurrences (`regularized_optimized.py::create_tikhonov_matrix_cached`, and both `regularized.py::create_tikhonov_matrix` closures).
- **Intercept and D/D_err not shown for several methods**: `intercept`/`intercept_se` from the Γ-vs-q² fit were computed but never written to the final results DataFrames for Cumulant A–D (`cumulant_analyzer.py`) or NNLS/Regularized (`laplace_analyzer.py`), so users had no way to judge whether `fit_through_origin` was appropriate. Now populated everywhere the fit is performed (including the Method B/C refine and Method D per-population/combined paths); `cumulant_results_dialog.py`'s summary and the NNLS/Regularized results tables display it. Cumulant A also gained a `Kurtosis` field (previously missing) and Cumulant B/A degrade gracefully to N/A where a moment genuinely isn't available from the underlying fit.
- **Dead "Adaptation Strategy" option removed** (`ade_dls/gui/dialogs/cumulant_dialog.py`, `ade_dls/analysis/cumulants_C.py`): the Cumulant C dialog offered "Individual / Global / Representative" adaptation strategies, but `get_adaptive_initial_parameters()` ignored everything except "individual". The dropdown and the dead `strategy` parameter were removed rather than silently implemented, to avoid misleading users; behaviour is unchanged (always per-dataset individual pre-fitting).
- **In-app documentation gaps**: the Inspector panel's "📖 Docs" tab (`ade_dls/gui/widgets/inspector_panel.py`) listed Cumulant Methods A–C but not D; added `get_cumulant_d_docs()`. `docs/analysis_methods.md` was missing a "Method A — ALV Software Cumulants" section entirely; added.

### Added

- **Intercept-significance quality assessment** (`ade_dls/gui/core/quality_assessment.py`, new module): `intercept_assessment(intercept, intercept_se)` flags whether a Γ-vs-q² intercept is statistically indistinguishable from zero (|intercept| < 2×SE → "fit through origin plausible", green) or significantly non-zero ("keep free intercept", red/no color), shown as a color-coded Intercept column/tooltip in the Cumulant, NNLS, and Regularized results dialogs.
- **Provenance now records the new fit options**: `fit_through_origin` is included in the Cumulant A/B pipeline step parameters (Cumulant C/D already carried it nested in their params dict). `TransparentPipeline.add_post_refinement_step()` gained an `extra_params` argument, used by the Regularized post-refinement step (`fit_through_origin`, clustering settings incl. `min_abundance`, per-population q² ranges) and the Method D post-refinement step (clustering/mode params) so these choices are traceable in the FAIR provenance record.
...
[3.3.0]: https://github.com/traianuschem/JADE-DLS/compare/v3.2.2...v3.3.0
[3.2.2]: https://github.com/traianuschem/JADE-DLS/compare/v3.2.1...v3.2.2
[3.2.1]: https://github.com/traianuschem/JADE-DLS/compare/v3.2.0...v3.2.1
[3.2.0]: https://github.com/traianuschem/JADE-DLS/compare/v3.1.1...v3.2.0
[3.1.1]: https://github.com/traianuschem/JADE-DLS/compare/v3.1.0...v3.1.1
[3.1.0]: https://github.com/traianuschem/JADE-DLS/compare/v3.0.0...v3.1.0
